...
# 0. Pre-flight (from a DIFFERENT cp node)
...
# 1. Fix NTP FIRST (still on 1.13 → legacy form, Google IP = no NTS)
...
# 4. Upgrade 1.13.10 → 1.14.1 (repeat step 0 first)

talosctl upgrade --nodes <node> --image factory.talos.dev/metal-installer/<schematic>:v1.14.1

# 5. Recover (expect v1.14.1) + Longhorn noexec check
...
# 6. Migrate config — ONLY after 1.14.1, use edit (not patch)

talosctl -n <node> edit machineconfig --mode=no-reboot

# DELETE from machine:  sysctls, kernel, install, time

# KEEP v1alpha1:        kubelet(Longhorn), network, cluster.allowSchedulingOnControlPlanes

# cluster.network.cni(flannel), controllerManager, scheduler, proxy, etcd
